...
min_val_norm <- data.frame(matrix(ncol = 2, nrow = 0))

for (n in 1:length(cover_min_values_beta)){
  min_val_norm[n,] <- c((cover_min_values_obs/total_obs)[n], (cover_min_values_beta/total_beta)[n])
}
```

```{r, include = FALSE}
total_obs_max <- sum(cover_max_values_obs)
total_beta_max <- sum(cover_max_values_beta)

max_val_norm <- data.frame(matrix(ncol = 2, nrow = 0))
max_val_norm[1,] <- c(simpson(cover[max_v,4:ncol(cover)]), simpson(beta_binomial_cover[max_v,4:ncol(beta_binomial_cover)])) 

for (n in 1:length(cover_max_values_beta)){
  max_val_norm[n+1,] <- c((cover_max_values_obs/total_obs_max)[n], (cover_max_values_beta/total_beta_max)[n])
}
```
...
Another comparison we can make is to look at how the variance of species diversity changes for a site when we apply the beta binomial cover update function and use the Hill Shannon diversity as a measure. We only look at sites with at least 10 plots.
...
```{r,  echo = FALSE, warning=FALSE}
aggregate_data2 <- aggregate_data[aggregate_data$observations > 9,]

ggplot(data = aggregate_data2, mapping = aes(y = `Beta binomial Hill Shannon` , x = `observed hill shannon`, colour = observations ))+
  scale_color_gradient(low="blue", high="red") +
  geom_point() +
  geom_abline(color = "blue") +
  ylab("variance of the beta binomial Hill Shannon diversity ") +
  xlab("variance of the observed Hill Shannon diversity") +
  ggtitle( "Comparison of variance for plots within each site")+xlim(0,13)+ylim(0,17) +
  labs(colour = "Number of plots \n in a site") +
  theme(plot.title = element_text(size=12))
```
