...
This page includes two examples of applications of the beta binomial cover update function presented at the [Bayesian model](Model.html) page. The page should, hopefully, give the reader a further feeling of how the beta binomial cover update function work as well as giving inspiration to use cases where the function can be applied.

The first example works with the tertiary habitat "Nardus grasslands". First, we will study the effect of the beta binomial cover update function on the data viewed through different diversity measures. This will primarily be done by visual inspections. Lastly, the example will include a small study of the relation between pH value of the soil and species diversity. 

The second example works with the tertiary habitat "Dry calcareous grasslands". This tertiary habitat is included to study the effect of nitrogen deposition on species diversity. In this example we also look at the effect of the beta binomial cover update function at site level as opposed to plot level as we only did in the first example.
...
So far have we just been comparing the effect of the beta binomial cover update function at a plot level. Since each plot is just a sample of a larger site, it could interesting to see the effect of the function at a site level instead.

We will do this be aggregating the data for all the plots from the same site. We will use the Hill Shannon diversity to measure the species diversity as this estimator does not favor neither common or rare species.
...
An other comparison we can make is to look at how the variance of species diversity changes for a site when we apply the beta binomial cover update function and use the Hill Shannon diversity as a measure.
...
The plot above suggest that for the sites with a large variance in Hill Shannon diversity for plots within the site, the variance become larger when the beta binomial cover update function is applied as opposed to the observed cover data. For sites with a low variance the change does not seem to be as big.
