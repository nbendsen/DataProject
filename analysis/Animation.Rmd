---
title: "Animation for model validation"
...
```{r}
animation <- ggplot(data = test_long, mapping = aes(x= scale_1, y = index)) + 
  geom_point() +
  geom_abline(colour = "blue") +
  transition_states(factor(scale), transition_length = 1, state_length = 1) +
  labs(title = "scale: {next_state}")


animate(animation,  nframes = 2*length(unique(test_long$scale)), duration = 20)
```


```{r}
animation2 <- ggplot(data = test_long, mapping = aes(x= scale_1, y = index, group = plot)) + 
  geom_point() +
  geom_abline(colour = "blue") +
  transition_states(factor(scale), transition_length = 2, state_length = 0) +
  labs(title = "scale: {next_state}")
