...
$$
<br>
In the bayesion framework the binomial model is used as a likelihood function.
<br>
...
The above theoretical discussions of the beta-binomial model are implemented in the function "method". The function takes as input a cover dataset and a presence/absence dataset and output a dataframe with the updated abundances for each specie in each plot of the input cover dataset. For further specifications of the function click [link](function_shannon.html)
...
```{r}
curve(dbeta(x,a,b), xlab = "relative abundance")
```
...
An other way of comparing two indexes is to plot them against each other.


```{r, echo = FALSE}
ggplot(data = result, mapping = aes(x = row_shannon, y = shannon))+
  geom_point() +
  geom_abline(color = "blue") +
  ylab("Our shannon index") +
  xlab("Simple row shannon index") +
  ggtitle("Our shannon index compared with the simple row shannon") +
  geom_smooth(lm = "smooth", col = "red")
```
