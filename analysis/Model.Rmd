...
* The function returns a dataframe, similar to the cover data, but now containing new abundance data calculated using the method described above. 
...
It can be seen that *"Plot 2"* has 0 observations for all species in the cover data, but has a *"1"* in the present/absence data for *"Specie 2"*, indicating that the specie is present in the plot, but is not hit by any of pins. In the beta binomial cover dataset *"Plot 2"* has a small value of 0.2155853 for *"Specie 2"*.

Because the specie is present in the plot but not hit by any number of pins, the beta binomial update model assigns a small value to it. If a specie is not present in the plot, then will the beta_binomial_cover_update function not change the value, and it will stay at 0 , this can be seen for the other species in *"Plot 2"*. 

When the specie is present, but not observed in the cover data, will $n = 16$ and $Y = 0$, this means that the posterior becomes: 

$$
P(\theta | Y = y) \sim Beta(\alpha + y, \beta + n-y)=Beta(\alpha + 0, \beta + 16-0)=Beta(\alpha , \beta + 16 )
$$

The opposite is seen for *"plot 7"*  with *"specie 1"*. This has 16 in the cover data, but only 15,59 in the beta-binomial cover dataset. This is because the prior distribution of the specie also has plays role and contribute to the posterior distribution, hence both the parameters for the likelihood and the prior contributes to the parameters of the posterior. When the number of pins that have hid a specie $y$ (in this case 16), is equal to the number of pins that was used $n$ (in this case 16), the parameters of the likelihood distribution will not contribute to the $\beta$ parameter of the posterior distribution. Thereby it will only be $y$ that contributes to $\alpha$. This is seen below: 

$$
P(\theta | Y = y) \sim Beta(\alpha + y, \beta + n-y)=Beta(\alpha + 16, \beta + 16-16)=Beta(\alpha + 16 , \beta )
$$
This has the the effect of lowering the cover value, in the plots with a high cover value.

A more detailed description of the implementation of the model can be found in [detailed description](bayesian_method_describtion.html). This page also includes the full code of the function.
