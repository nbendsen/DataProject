...
The presentation of each function will also contain a small example of how the function is applied. To do this, we again use a small constructed cover dataset and presence/absence dataset that was presented at the [Introduction](introduction.html) page. We remind the reader that cover data describes how much of the surface area that would be covered by a specific species if it is projected down on the ground. In this example cover data will take the form of an integer value between 0 and 16, where 16 is full cover and 0 is no cover.
...
Main function of this vignette. The function is an implementation of the method outlined on the [Bayesian model](Model.html) page.
...
This function takes in two data frames with cover data, one with the observed cover data, and one with the beta binomial cover data. Its output is a plot that shows different hill diversities as a function of the exponent $l$ in:
...
* **data_new**: Data frame with plots as rows and species as columns. Each cell in the data frame contains the beta binomial cover data for a given specie in a given plot.
