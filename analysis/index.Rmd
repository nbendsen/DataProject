...
```{r}
result_merged <- merge(result, abiotiske, by = c("site", "plot", "year"))
```


```{r}
plot_merge <- result[!(result$pH == "mv"),]
plot_merge$pH <- as.numeric(plot_merge$pH)

ggplot(plot_merge, aes(x = pH, y = shannon, color = factor(site)))+
  geom_point()+
  xlim(3,6.5)+
  theme_classic()+
