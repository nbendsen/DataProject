...
<br>
While the Shannon index often will give a better estimate of the diversity in a location, it requires available data of not only presence of a species but also of its abundance. In reality, such data are collected as samples of smaller areas meant to represent a larger location. In smaller areas, the abundance of each species present may be well sampled, but rarely do smaller areas contain all species present in the location. In this vignette, we will propose a new method with the goal to combine abundance (e.g cover) data often collected in smaller samples, with presence/absence data that is often collected over larger areas but lack the information of individual species’ abundance. Both types of data contain important information about the area that we would like to use to obtain more accurate diversity measures for a nature area. 
...
Within each site a number of "plots" are placed. It is data from these plots that we will be studying in this vignette. Below we zoom in on a single site. The red dots are intended to visualize the plots in this specific site.
...
Within this square the cover of each species is projected down on the surface. A number of pins are placed uniformly onto the projection. The cover data for each species is the number of pins it is hit by. Thus, the cover data is a sample of the 2 dimensional projection of each species down on the surface. In the NOVANA dataset 16 pins are placed uniformly within the square.
<br>
